...
- [x] 05-02-PLAN.md — Hand-authored DARLNG `D` favicon rasterized into `.ico` (hand-rolled container, no new dep) and a 180x180 apple-touch icon, plus `robots.txt` pointing at the sitemap index and an nginx `/sitemap.xml` route
- [x] 05-03-PLAN.md — Lighthouse against the built `dist/` served by the real `nginx.conf` (LCP/CLS/TBT green across four runs), impact-filtered axe scan on three pages, and the DEPLOY.md live-domain deferred checks
