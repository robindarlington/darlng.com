# Milestone v1.0: DARLNG Release Hub

**Status:** ✅ SHIPPED (locally complete) 2026-08-09
**Phases:** 1-5
**Total Plans:** 13

## Overview

Full build of the DARLNG artist release hub: dark cinematic fan-facing site with the Eseriani hero + zero-JS YouTube facade, discography, listen-everywhere pages, Listmonk-ready newsletter capture, and launch-grade SEO/perf — built and verified entirely locally, deploy-ready for the user's Coolify cutover via website/DEPLOY.md.

## Key Accomplishments

1. Astro 5 + Tailwind 4 + Preact scaffold with production Dockerfile/nginx pipeline, container-verified cache/404/security-header contracts, and a complete operator runbook (DEPLOY.md) for Coolify/Listmonk/Resend/DNS.
2. Original brand system: near-black warm base + single swappable turquoise jewel accent (#2DD9C5, derived from the Eseriani artwork), Unbounded/Manrope type, 9-pair WCAG AA contrast gate enforced in CI (`npm run check`).
3. Core fan experience: full-bleed cinematic hero (LCP-protected, contrast-safe scrim with published math), click-to-load youtube-nocookie facade (zero third-party requests pre-click), catalog grid, and branded /listen/[slug] pages for all four releases (22 platform links, zero invented logos).
4. Newsletter capture: Preact island with six browser-verified states, honeypot with timing-safe fake success, build-time env gating, mock-Listmonk local test harness — wired for two Coolify env vars at deploy time.
5. Launch polish: per-page OG/Twitter meta with generated 1200×630 cards, sitemap + robots + nginx /sitemap.xml route, hand-authored favicon set, Lighthouse green over production-faithful h2 (home mobile LCP 1980ms, CLS 0, TBT 0), axe zero violations.
6. Every phase: independent goal-backward verification, code review with auto-fix, security audit (ASVS L1 — 0 open threats across 55 verified threat instances), UI audits.

## Stats

- 5 phases, 13 plans, ~26 milestone commits (2026-08-06 → 2026-08-09 execution window)
- ~1,560 lines of site source (astro/ts/tsx/mjs/css) + deploy artifacts
- 6 static pages, 5 generated OG cards, 4 releases, 22 platform links
