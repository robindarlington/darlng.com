...
**Status:** ✅ All 20 v1 requirements complete (verified per-phase; see phase VERIFICATION.md files and v1.0-MILESTONE-AUDIT.md).
**Outcome notes:** FAN-01's ALTCHA clause amended 2026-08-08 (Listmonk public API does not support ALTCHA — replaced with honeypot + double opt-in + documented rate limiting, source-verified). Live-domain halves of INFRA-02/03/04, FAN-01 (live email), SEO-01 (live OG check), PERF-01 (live Lighthouse) are deferred-by-design to the user's DEPLOY.md cutover per the 2026-08-04 scope revision.
